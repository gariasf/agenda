...
module.exports = function(extraJob) {
  ...
  function jobQueueFilling(name) {
    ...
    self._findAndLockNextJob(name, definitions[name], (err, job) => {
      ...
      if (job) {
        debug('[%s] job locked while filling queue', name, job.attrs.name);
  ...
  function jobProcessing() {
    ...
    debug('[%s] about to process job', job.attrs.name, job.attrs.name);

    // If the 'nextRunAt' time is older than the current time, run the job
    // Otherwise, setTimeout that gets called at the time of 'nextRunAt'
    debug("Timed", job.attrs.nextRunAt, now)
    if (job.attrs.nextRunAt < now) {
      debug('[%s] nextRunAt is in the past, run the job immediately', job.attrs.name, job.attrs.name);
      runOrRetry();
    } else {
      const runIn = job.attrs.nextRunAt - now;
      debug('[%s] nextRunAt is in the future, calling setTimeout(%d)', job.attrs.name, job.attrs.name, runIn);
      ...
      if (self._processInterval) {
        if (jobDefinition.concurrency > jobDefinition.running && self._runningJobs.length < self._maxConcurrency) {
          ...
          if (job.attrs.lockedAt < lockDeadline) {
            debug('[%s] job lock has expired, freeing it up', job.attrs.name, job.attrs.name);
          ...
          debug('[%s] processing job', job.attrs.name, job.attrs.name);
          ...
          debug('[%s] concurrency preventing immediate run, pushing job to top of queue', job.attrs.name, job.attrs.name);
  ...
  function processJobResult(err, job) {
    ...
    if (self._runningJobs.indexOf(job) === -1) {
      debug('[%s] callback was called, job must have been marked as complete already', job.attrs.name);
