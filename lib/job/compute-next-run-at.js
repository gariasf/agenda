...
module.exports = function() {
  ...
  const computeFromInterval = () => {
    debug('[%s:%s] computing next run via interval [%s]', this.attrs.name, this.attrs.name, interval);
    ...
    try {
      ...
      debug('[%s:%s] nextRunAt set to [%s]', this.attrs.name, this.attrs.name, this.attrs.nextRunAt.toISOString());
    } catch (e) {
      // Nope, humanInterval then!
      try {
        if (!this.attrs.lastRunAt && humanInterval(interval)) {
          this.attrs.nextRunAt = lastRun.valueOf();
          debug('[%s] nextRunAt set to [%s]', this.attrs.name, this.attrs.nextRunAt.toISOString());
        } else {
      ...
      if (isNaN(this.attrs.nextRunAt)) {
        this.attrs.nextRunAt = undefined;
        debug('[%s] failed to calculate nextRunAt due to invalid repeat interval', this.attrs.name);
  ...
  function computeFromRepeatAt() {
    ...
    if (offset === date(repeatAt, offset).valueOf()) {
      this.attrs.nextRunAt = undefined;
      debug('[%s:%s] failed to calculate repeatAt due to invalid format', this.attrs.name, this.attrs.name);
